<x-app-layout>
    <div class="max-w-7xl mx-auto mt-10 px-4 sm:px-6 lg:px-8">
        <div class="my-4">
            ...
            <a href="{{ route('comments') }}" class="inline-block ml-4 py-2 px-4 btn btn-secondary text-decoration-none">
                {{ __('自分の投稿を確認する') }}
            </a>
        </div>
                ...
                <ul>
                    @foreach ($comments as $comment)
                        <li class="mb-6 bg-white border rounded-lg p-4">
                            <h3 class="text-lg font-bold mb-2 border-bottom">{{ $comment->comment_text }}</h3>
                            <p class="text-gray-1000 mt-4">{{ $comment->comment_text }}</p>
                            <div class="flex justify-between mt-8">
                                <p class="text-gray-600">{{ $comment->user_id->name }}</p>
